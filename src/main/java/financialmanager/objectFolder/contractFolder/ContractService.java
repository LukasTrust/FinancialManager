...
public class ContractService {
    ...
    public void checkIfTransactionsBelongToContract(List<Transaction> transactions) {
        ...
        List<Transaction> existingTransactions = transactionService.findByBankAccountIdAndNoContract(bankAccountId);
        if (!existingTransactions.isEmpty()) {
            transactionsWithoutContract.addAll(existingTransactions);
        }
    ...
    private void assignTransactionsToExistingContracts(List<Transaction> transactions, List<Contract> contracts) {
        for (Contract contract : contracts) {
            transactions.stream()
                    .filter(transaction -> isTransactionValidForContract(transaction, contract))
                    .forEach(transaction -> {
    ...
    private boolean isTransactionValidForContract(Transaction transaction, Contract contract) {
    ...
    private List<Contract> tryToFindNewContracts(List<Transaction> transactions) {
        if (transactions == null || transactions.isEmpty()) {
            return new ArrayList<>();
        }

        Map<Double, List<Transaction>> groupedByAmount = transactions.stream()
